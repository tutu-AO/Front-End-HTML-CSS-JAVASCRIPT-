...
$(document).ready(function(){
   ...
   $("#click").on("click", function (e) {
       ...
       if (x === true) {
         const proxyurl = "";
         const url = "https://api.unsplash.com/search/photos?query=random&per_page=25&client_id=HOpzu3kd4_rNxAORq9ONrizMkJUpt2Xz0S7dlBvvcIE";
         fetch(proxyurl+url)
   ...
   $('#my-search').keypress(function (event) {
      ...
      if (keycode == '13' && x === true) {
        ...
        localStorage.clear(); //Clears storage for next request
         ...
         const proxyurl = "";
         const url = "https://api.unsplash.com/search/photos?query="+searchVal+"&per_page=25&client_id=HOpzu3kd4_rNxAORq9ONrizMkJUpt2Xz0S7dlBvvcIE";
         fetch(proxyurl + url)
            .then((response) => response.text())
            .then(function (contents) {
              localStorage.setItem("userObject", contents);
